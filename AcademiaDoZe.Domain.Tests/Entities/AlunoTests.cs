...
public class AlunoTests
{
    ...
    [Theory(DisplayName = "Aluno: criação bem-sucedida")]
    ...
    [Theory(DisplayName = "Aluno: nome vazio")]
    [InlineData("")]
    [InlineData(" ")]
    [InlineData("   ")]
    ...
    [Theory(DisplayName = "Aluno: data nascimento inválida")]
    [InlineData("default", "DATA_NASCIMENTO_OBRIGATORIO")]
    [InlineData("under12", "DATA_NASCIMENTO_MINIMA_INVALIDA")]
    [InlineData("under5", "DATA_NASCIMENTO_MINIMA_INVALIDA")]
    public void Deve_Falhar_Criacao_Quando_DataNascimentoInvalida(string scenario, string expectedMessage)
    {
        DateOnly dataParam = scenario switch
        {
            "default" => default,
            "under12" => DateOnly.FromDateTime(DateTime.Today.AddYears(-10)),
            _ => DateOnly.FromDateTime(DateTime.Today.AddYears(-5))
        };

        var result = Aluno.Criar(1, "João", "529.982.247-25", dataParam, "(11) 91234-5678", "user@example.com", GetValidLogradouro(), "123", "", "Abcdef", GetValidArquivo());
        Assert.True(result.IsFailure);
