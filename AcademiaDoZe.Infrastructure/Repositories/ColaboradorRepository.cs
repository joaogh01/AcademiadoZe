...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
        ...
        try
        {
            string query = @"
                UPDATE tb_colaborador SET 
                    cpf = @Cpf, nome = @Nome, nascimento = @Nascimento, telefone = @Telefone, email = @Email, 
    ...
    public async Task<IEnumerable<Colaborador>> ObterPorVinculo(ColaboradorVinculo vinculo, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InfrastructureException("ERRO_OBTER_POR_VINCULO", $"Erro ao obter colaboradores por vinculo {vinculo}: {ex.Message}", ex);
        }
    }
