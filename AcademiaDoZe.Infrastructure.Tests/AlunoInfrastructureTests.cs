...
public class AlunoInfrastructureTests : TestBase
{
    ...
    internal static async Task<Aluno> CriarEInserirAlunoAsync(AlunoRepository alunoRepo, LogradouroRepository logradouroRepo, DatabaseType dbType)
    {
        ...
        var foto = Arquivo.Criar(new byte[] { 10, 20, 30, 40 }).Value!;
    ...
    [Fact]
    public async Task InserirAlunoParaPrint()
    {
    ...
    public async Task Aluno_Atualizar_Sucesso()
    {
        ...
        var aluno = await _alunoRepo.Adicionar(Aluno.Criar(
            0, "João Original", GerarCpf(), new DateOnly(2000, 10, 20),
            GerarTelefone(), GerarEmail(), logradouro, "100", "Santos",
            $"Senha123{DatabaseType}", foto).Value!);
        ...
        Assert.Equal("250", noBanco.Endereco.Numero);
    }
